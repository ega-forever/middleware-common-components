...
class MasterNode {
   ...
   * @param serviceName [from amqplib] the rabbitmq service name
   *
   * @memberOf MasterNode
   */
  constructor (_channel, serviceName) {
    this._myid = uniqid();
    this._queues = {
      findMasterQueue: `${serviceName}_findMaster_${this._myid}`,
      findMasterRoute: `${serviceName}_findMaster`,
      setMasterQueue: `${serviceName}_setMaster_${this._myid}`,
      setMasterRoute: `${serviceName}_setMaster`
    };
